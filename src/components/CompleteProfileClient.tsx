...
import { fetchUserProfile, saveUserProfile } from '@/lib/firestoreProfile'
...
export default function CompleteProfileClient() {
  ...
  useEffect(() => {
    const unsubscribe = auth.onAuthStateChanged(async (user) => {
      if (!user) {
        const redirectUrl =
          `/auth/login?next=/complete-profile` +
      ...
      try {
        ...
        if (profile) {
          if (profile.displayName) {
            router.replace(nextParam || '/discover')
            return
          }
          reset({
        ...
        console.error('[CompleteProfileClient] getUserProfile error:', e)
      } finally {
        setLoading(false)
      }
    })
  ...
  const onSubmit = async (data: FormValues) => {
    ...
    try {
      ...
      // Save to both profile systems for consistency
      await Promise.all([
        upsertUserProfile(user.uid, data.displayName, data.summary),
      ...
      console.error('[CompleteProfileClient] upsert error:', err)
